 ...
 * now the book had nowhere to put it: `Payment` is money coming in, a credit note
 * moves an account without any money at all, and an expense is joined to nobody.
 * So the four hundred riyals the owner lent Ahmed on Tuesday existed only in the
 * owner's head, while every other figure in the shop was written down.
 *
 * **It is money out that makes a debt** — the exact mirror of [Payment], which is
 * money in that settles one. Lending four hundred to somebody who owes eight
 ...
 * note each come out of a numbered book the shop keeps, and a hand of cash across
 * a counter comes out of no book at all. [note] is where "for the school fees"
 * goes if the owner wants it there.
 */
@Serializable
data class Loan(
    ...
    /**
     * Always positive. The store clamps it. A negative loan would be a payment,
     * and a payment is its own type.
     */
    val amount: Double,
    @Serializable(with = InstantSerializer::class)
    val lentAt: Instant = Timestamps.now(),
    /**
     ...
     * Absent rather than empty when skipped.
     */
