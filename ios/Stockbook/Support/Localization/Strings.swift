...
struct Strings {
    ...
    // MARK: - Search: the way in that no list gives

    /// What the box will take, rather than the word "Search".
    ///
    /// The four lists in the book each answer "what happened in August". This
    /// answers "where is this piece of paper", and an owner has no reason to
    /// guess that a receipt number typed here will find anything — so the
    /// placeholder says so.
    var searchRecords: String {
        pick("Invoice, receipt, name or amount", "ಬಿಲ್, ರಸೀದಿ, ಹೆಸರು ಅಥವಾ ಮೊತ್ತ")
    }

    var nothingMatches: String { pick("Nothing matches that", "ಅದಕ್ಕೆ ಏನೂ ಹೊಂದುವುದಿಲ್ಲ") }
    ...
    var billLabel: String { pick("Bill", "ಬಿಲ್") }
    var voucherLabel: String { pick("Voucher", "ವೋಚರ್") }
    var expenseLabel: String { pick("Expense", "ಖರ್ಚು") }
    ...
    // MARK: - The receipt for a payment
    //
    // The half-page the shop tears off and hands over. Worded from the
    // customer's end, not the owner's: they are holding it, and everything on it
    // is about money they just parted with.

    var paymentSaved: String { pick("Payment saved", "ಪಾವತಿ ಉಳಿಸಲಾಗಿದೆ") }
    ...
    /// customer who has lost their slip is the whole reason to print a second
    /// one.
    var viewReceipt: String { pick("View receipt", "ರಸೀದಿ ನೋಡಿ") }

    /// Set against the letterhead: what the piece of paper is.
    ///
    /// A shop paying its own supplier receives nothing, so that page is a
    /// voucher — the word the supplier's own book would use for it. One word for
    /// both would be read from the wrong end on one of the two.
    var paymentReceipt: String { pick("Payment Receipt", "ಪಾವತಿ ರಸೀದಿ") }
    var paymentVoucher: String { pick("Payment Voucher", "ಪಾವತಿ ವೋಚರ್") }

    var receivedFrom: String { pick("Received from:", "ಸ್ವೀಕರಿಸಿದ್ದು — ಇವರಿಂದ:") }
    var paidTo: String { pick("Paid to:", "ಪಾವತಿಸಿದ್ದು — ಇವರಿಗೆ:") }
    var amountPaid: String { pick("Amount paid", "ಪಾವತಿಸಿದ ಮೊತ್ತ") }

    /// The three lines under the figure, and always all three.
    ///
    /// "Previous balance" rather than the statement's "Opening balance": there
    /// is no period here to open, and the reader's question is what they owed a
    /// minute ago.
    var accountAfterThisReceipt: String {
        pick("The account after this receipt", "ಈ ರಸೀದಿಯ ನಂತರ ಖಾತೆ")
    }
    var previousBalance: String { pick("Previous balance", "ಹಿಂದಿನ ಬಾಕಿ") }
    var balanceNow: String { pick("Balance now", "ಈಗಿನ ಬಾಕಿ") }

    func receiptFileName(_ name: String, _ date: String) -> String { "receipt-\(name)-\(date).pdf" }
    ...
    /// to that party and handed across the counter. The register behind
    /// `reportButton` lists everybody, and showing it to a customer would be a
    /// breach rather than an untidy page — see the note on `SummaryDocument`.
    var reportButton: String { pick("Report", "ವರದಿ") }
    var summaryButton: String { pick("Summary", "ಸಾರಾಂಶ") }
    ...
    var loanLabel: String { pick("Loan", "ಸಾಲ") }

    /// The figure on a statement: what was handed over across the period.
    var lentInPeriod: String { pick("Lent", "ಕೊಟ್ಟ ಸಾಲ") }

    /// The two directions money can go with a customer, on the one sheet that
    /// records both.
    ///
    /// Named for the direction rather than for the record — "Received" and
    /// "Given" — because the owner tapping one is answering "which way did the
    /// money go", and that is a question about the counter rather than about the
    /// file.
    var moneyReceived: String { pick("Received", "ಬಂದದ್ದು") }
    var moneyGiven: String { pick("Given", "ಕೊಟ್ಟದ್ದು") }
