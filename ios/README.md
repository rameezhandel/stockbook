...
**Requirements:** Xcode 14.2+, iOS 16.0+, iPhone only, portrait, dark-only.
...
```
ios/
├── Stockbook.xcodeproj        generated by tools/make_xcodeproj.py
...
**The project file is generated, not hand-maintained.** Xcode 14 predates
synchronized folder groups, so every source file has to be listed explicitly —
exactly the tedium that lets a pbxproj rot. After adding or moving a file:

```sh
python3 ios/tools/make_xcodeproj.py
```

It derives object ids from file paths, so regenerating produces the same file
rather than a diff of shuffled identifiers.

**Why Xcode 14 and iOS 16.** The project originally targeted iOS 17 with
SwiftData, which requires Xcode 15, which requires macOS Ventura. That chain
excluded the machine it was being built on. Storage moved behind a protocol
first, so the version change was mostly `@Observable` → `ObservableObject`,
`@Query` → store reads, and Swift Testing → XCTest.
